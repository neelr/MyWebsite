...
export default function Home({ quote, spotifyData, date, ...props }: { quote: quoteType, spotifyData: spotifyType, date: number }) {
  let [spotifyProgress, setProgress] = useState(spotifyData.progress);
  let [spotifyDataS, setSpotifyDataS] = useState(spotifyData);
  useEffect(() => {
    let x = setInterval(() => {
      fetch('/api/spotify').then(res => res.json()).then(d => {
        date = Date.now()
  ...
  return (
    <Box sx={{ display: "flex", mt: "100px" }}>
      <Box sx={{
        ...
        <hr />
        {spotifyDataS.duration > 0 ? (<><Text as="p">Listening on {spotifyDataS.device.name} to:</Text>
          <Link href={spotifyDataS.url} target="_blank" rel="noopener noreferrer" sx={{ display: "flex", flexDirection: "column" }}>
            <Box sx={{ display: "flex" }}>
              <img src={spotifyDataS.albumArt} width={40} height={40} alt={spotifyDataS.album} />
              <Text my="auto" mx="10px">{`${spotifyDataS.song}, ${spotifyDataS.artist}`}</Text>
              <Text my="auto">{`${Math.floor((spotifyProgress / 1000) / 60)}:${pad(Math.floor(spotifyProgress / 1000 % 60))} / ${Math.floor((spotifyDataS.duration / 1000) / 60)}:${pad(Math.floor(spotifyDataS.duration / 1000 % 60))}`}</Text>
            </Box>
          </Link></>) : ""}
        <Box>
